...
static void ibus_rime_create_session (IBusRimeEngine *rime_engine) {
	rime_engine->session_id = rime_api->create_session();
	Bool inline_caret = g_ibus_rime_settings.embed_preedit_text
...
static void ibus_rime_engine_update(IBusRimeEngine *rime_engine) {
	...
	if (
		...
		if (has_highlighted_span) {
			...
			if (g_ibus_rime_settings.color_scheme) {
				...
				ibus_attr_list_append(
					inline_text->attrs,
					ibus_attr_foreground_new(g_ibus_rime_settings.color_scheme->text_color, start, end)
				);
	...
	if (context.menu.num_candidates) {
		...
		//show page down except last page
		if (has_page_down) {
			ibus_lookup_table_append_candidate(rime_engine->table, ibus_text_new_from_string(""));
		}
...
static gboolean ibus_rime_engine_process_key_event (IBusEngine *engine, guint keyval, guint keycode, guint modifiers) {
	...
	gboolean result = rime_api->process_key(rime_engine->session_id, keyval, modifiers);
