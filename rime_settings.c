...
static void select_color_scheme(struct IBusRimeSettings *settings, const char *color_scheme_id) {
...
void ibus_rime_load_settings() {
	...
	if (rime_api->config_get_bool(&config, "style/inline_preedit", &inline_preedit)) {
		g_ibus_rime_settings.embed_preedit_text = !!inline_preedit;
	}

	const char *preedit_style_str = rime_api->config_get_cstring(&config, "style/preedit_style");
	...
	const char *cursor_type_str = rime_api->config_get_cstring(&config, "style/cursor_type");
	if (cursor_type_str) {
	...
	const char *color_scheme = rime_api->config_get_cstring(&config, "style/color_scheme");
	if (color_scheme) {
